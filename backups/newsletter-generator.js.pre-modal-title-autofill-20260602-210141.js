...
function openModal(category) {
    ...
    if (category === 'funFact') {
        modalTitleText = 'Choose Fun Fact';
        data = funFacts || [];
    } else if (category === 'proTip') {
        modalTitleText = 'Choose Pro Tip';
        data = proTips || [];
    } else if (category === 'quote') {
        modalTitleText = 'Choose Motivational Quote';
        data = motivationalQuotes || [];
    }

    if (title) title.innerText = modalTitleText;
    ...
    if (list) {
        ...
        randomLi.innerHTML = `<i class="fas fa-dice"></i> <span>Pick a Random ${modalTitleText.replace('Choose ', '')} for me</span>`;
    ...
    if (search) {
        search.placeholder = `Search ${modalTitleText.toLowerCase()}...`;
        search.oninput = () => {
...
function copyForOutlook() {
  ...
  function fillPersonalFromProfile() {
    ...
    if (p.hobbies && p.hobbies.length) parts.push(`Lately I've been enjoying ${p.hobbies.slice(0,2).join(' and ')}.`);
    if (p.personality) {
      const pers = String(p.personality);
      parts.push(`As someone who's ${pers.toLowerCase()}, I'm always looking for ways to help families like yours.`);
    }
    if (p.goals) parts.push(p.goals);
    if (p.challenges) {
      const ch = Array.isArray(p.challenges) ? p.challenges.join(', ') : p.challenges;
      parts.push(`Helping with things like ${String(ch).toLowerCase()}.`);
    }
