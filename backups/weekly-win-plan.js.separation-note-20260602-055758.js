 ...
 *
 * Contains code for TWO SEPARATE TOOLS (kept in one file for related "plan your week / plan your year" strategy features):
 *
 * 1. WEEKLY WIN PLAN (prospecting time blocks):
 *    - Function: generateWeeklyPlan()
 *    - Button: #generate-win-plan-btn (in #weekly-win-plan section)
 *    - Output: #weekly-tasks-container + #weekly-plan-results
 *    - Loading: Replaces #global-loading content with custom weekly UI (backup/restore in finally)
 *    - Prompt: 7-day JSON plan based on userSetup (hours, focus, hobbies, activities)
 *
 * 2. 2026 BUSINESS PLAN (full year strategy + AI plan):
 *    - Function: generatePlan(targetOutputId = 'plan-output')
 *    - Button: #generate-plan-btn (in #planning section)
 *    - Output: #plan-output (rich wrapper with Power Theme, milestones, Execution Hub linking to other tools)
 *    - Loading: Uses showLoadingWithTips(businessTips) + injects #plan-enrich-panel (live form snapshot + animated progress + steps)
 *    - Heavily uses central profile, live insight, hobby tactics, inspiration pull, style cards, baseline save, etc.
 *    - Prompt: Long personalized 2026 plan with specific structure (Power Theme, Quarterly Milestones, Tool Ties, etc.)
 *
 * CRITICAL: These are NOT the same. Do not mix:
 *   - Button IDs
 *   - Function names/calls (generatePlan vs generateWeeklyPlan)
 *   - Output containers (#plan-output vs weekly containers)
 *   - Loading strategies (enrich panel vs full custom replace)
 *   - Form data (the business planning form with .hobby-checkbox etc. vs the weekly prefs accordion)
 *
 * Shared: Some profile helpers (getCentralProfile, getEffectiveSetup, userSetup), the global loading overlay, localStorage patterns.
 *
 * Extracted from monolithic index.html. Self-initializes. Exposes public API on window.
 *
 * SPECIAL NOTE (per user request): Always keep these two completely separate in future edits. Verify both buttons, both progress experiences, profile sync/restore/save for business plan, and weekly persistence work independently after any change.
 */
...
function restoreSavedBusinessPlan() {
    ...
    if (saved && output && output.innerHTML.trim() === '') {
        // Wrap legacy saved content in the new premium chrome so it doesn't look broken
        output.innerHTML = `
          <div class="bg-white dark:bg-gray-900 border-2 border-[#F15A29]/30 rounded-3xl shadow-2xl p-8 md:p-10 mt-4">
            <div class="flex items-center justify-between mb-4">
              <div>
                <div class="inline px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold">RESTORED PREVIOUS PLAN</div>
                <h3 class="text-2xl font-bold text-[#F15A29] mt-2">Your previously generated 2026 Plan</h3>
              </div>
              <button onclick="document.getElementById('plan-output').innerHTML=''; localStorage.removeItem('savedBusinessPlan');" class="text-xs px-4 py-2 rounded-2xl border text-red-500 hover:bg-red-50">Clear this</button>
            </div>
            <div class="text-sm text-gray-500 mb-3">This is an older saved version. For the best experience (new Power Theme, Low-Anxiety Starter Kit, and direct toolkit links), hit the big generate button again with your latest profile + numbers.</div>
            <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl p-6" id="plan-preview">
              ${saved
                .replace(/onclick="showSection\('social-media-strategy'\)"/g, `onclick="window.showSection('social')"` )
                .replace(/onclick="showSection\('referral-partners'\)"/g, `onclick="window.showSection('referrals')"` )
                .replace(/onclick="showSection\('social-media-strategy'\)"/g, `onclick="window.showSection('social')"` )
                .replace(/onclick="showSection\('referral-partners'\)"/g, `onclick="window.showSection('referrals')"` )
              }
            </div>
        ...
        // Post-process to fix any stale onclick handlers from previously saved plans
        // (e.g. old 'social-media-strategy' and 'referral-partners' that cause blank page)
        setTimeout(() => {
          const preview = document.getElementById('plan-preview');
          if (preview) {
            preview.querySelectorAll('[onclick*="social-media-strategy"], [onclick*="referral-partners"]').forEach(el => {
              let oc = el.getAttribute('onclick') || '';
              oc = oc.replace(/social-media-strategy/g, 'social');
              oc = oc.replace(/referral-partners/g, 'referrals');
              // Ensure window. prefix for reliability in injected HTML
...
async function generateWeeklyPlan() {
    // WEEKLY WIN PLAN ONLY - see top of file for separation from generatePlan / 2026 Business Plan
...
function restoreBusinessPlanningForm() {
    ...
    if (savedHobbies) {
        ...
        // If no saved form state, default to profile's hobbies for consistency
        const prof = (typeof window.getUserProfile === 'function') ? window.getUserProfile() : {};
        const profileHobbies = prof.hobbies || [];
        document.querySelectorAll('.hobby-checkbox').forEach(cb => {
            if (profileHobbies.includes(cb.value)) cb.checked = true;
        });
        const ho = document.getElementById('hobby-other');
        if (ho && prof.hobbiesOther) ho.value = prof.hobbiesOther;
    }
    ...
    if (savedActivities) {
        ...
        // Default to profile's activities
        const prof = (typeof window.getUserProfile === 'function') ? window.getUserProfile() : {};
        const profileActivities = prof.activities || [];
        document.querySelectorAll('.activity-checkbox').forEach(cb => {
            if (profileActivities.includes(cb.value)) cb.checked = true;
        });
    }
}
...
// Save the entire generated plan as a rich item in My Saved Items (unified vault)
function saveFullPlanToVault() {
    try {
        ...
        // Prefer rich HTML (from marked) so the saved plan renders beautifully with headings/lists when viewed later
        const richContent = (preview ? preview.innerHTML : (output ? (output.querySelector('#plan-preview') ? output.querySelector('#plan-preview').innerHTML : output.innerHTML) : '')) || 'Custom 2026 Business Plan';
        const title = '2026 Business Plan — ' + (document.querySelector('input[name="plan-style"]:checked')?.value || 'Custom');

        const contentToStore = richContent.length > 12000 ? richContent.substring(0, 12000) + '<!-- truncated for storage -->' : richContent;
        ...
        if (typeof window.toggleSaveIdea === 'function') {
            ...
            // Fallback direct to storage (matches other save patterns)
            let saved = [];
            try { saved = JSON.parse(localStorage.getItem('socialSavedIdeas') || '[]'); } catch(e){}
            const already = saved.some(s => s.title === title);
            if (!already) {
                saved.push({
                    title: title,
                ...
                if (typeof window.showSavedFeedback === 'function') {
                    ...
                    alert('2026 Business Plan saved to My Saved Items!');
                }
            } else {
...
// ============================================================
// NEW: Fun, low-anxiety helpers for the refreshed 2026 Business Plan
// ============================================================

window.applyPlanPreset = function(preset) {
  // Sets realistic numbers + style + a little personality flavor
  const styleRadios = document.querySelectorAll('input[name="plan-style"]');
  ...
  if (preset === 'conservative') {
    ...
    if (styleRadios[1]) styleRadios[1].checked = true; // Database Reactor feels sustainable
    ...
    if (styleRadios[2]) styleRadios[2].checked = true; // Balanced
    ...
    if (styleRadios[0]) styleRadios[0].checked = true; // Referral Mastery
    ...
    if (styleRadios[0]) styleRadios[0].checked = true;
    ...
    if (styleRadios[2]) styleRadios[2].checked = true;
    // Check a couple hobby-ish boxes to signal the vibe
    const fam = document.getElementById('hobby-family'); if (fam) fam.checked = true;
    const out = document.getElementById('hobby-outdoors'); if (out) out.checked = true;
  }

  // Trigger live insight update
  setTimeout(() => {
  ...
  if (typeof window.showToast === 'function') {
    window.showToast('Preset loaded — tweak anything you want');
  }
};
...
// Save current entire form state as "My Baseline" for quick reuse / consistency across plans
...
// Load previously saved baseline (if exists)
...
// Render additional profile fields (challenges, personality, partner types, family, tone) into the planning page for full visibility/consistency
...
function wirePlanLiveCalculations() {
  const fields = ['target-closings', 'target-income', 'closing-ratio', 'current-partners'];
  fields.forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('input', () => updatePlanLiveInsight());
      el.addEventListener('change', () => updatePlanLiveInsight());
    }
  });

  // Also react to style changes a little - use dedicated note (prevents repeating text accumulation)
  document.querySelectorAll('input[name="plan-style"]').forEach(r => {
    r.addEventListener('change', () => {
      const note = document.getElementById('plan-style-note');
      if (note) {
        note.textContent = '(style updated — regenerate when ready)';
        // auto-clear after a few seconds so it doesn't linger forever
        setTimeout(() => { if (note) note.textContent = ''; }, 4500);
      }
      // ensure visual highlight updates immediately
      if (typeof window.wirePlanStyleCards === 'function') window.wirePlanStyleCards();
    });
  });

  // Initial calculation if values are pre-filled by restore
  setTimeout(updatePlanLiveInsight, 180);
  ...
  function updatePlanCompleteness() {
    ...
    const base = Math.round((filled / checks.length) * 70);
  ...
  // Visual active state for the pretty plan-style cards
  function wirePlanStyleCards() {
    document.querySelectorAll('.plan-style-card').forEach(card => {
      const radio = card.querySelector('input[type="radio"]');
      if (!radio) return;
      const update = () => {
        document.querySelectorAll('.plan-style-card').forEach(c => c.classList.remove('!border-[#00A89D]', 'ring-2', 'ring-[#00A89D]/30', 'bg-[#00A89D]/5'));
        if (radio.checked) card.classList.add('!border-[#00A89D]', 'ring-2', 'ring-[#00A89D]/30', 'bg-[#00A89D]/5');
      };
...
// =====================================================
// ROBUST BUTTON WIRING (delegation + direct to survive clones/replaces)
// =====================================================
...
})(); // This closes the outer IIFE for the entire weekly-win-plan.js module. 
// NOTE: This file contains code for TWO SEPARATE TOOLS to keep related strategy/planning code together:
// 1. Weekly Win Plan (generateWeeklyPlan + time blocks + tiles) - for the #weekly-win-plan section, button #generate-win-plan-btn
// 2. 2026 Business Plan (generatePlan + form + profile sync + inspiration + hobby tactics + Execution Hub) - for the #planning section, button #generate-plan-btn
// They share some profile helpers and the loading overlay but have completely separate entry points, prompts, output rendering, and button wirings.
// Do not mix their button IDs, function calls, or output containers.
