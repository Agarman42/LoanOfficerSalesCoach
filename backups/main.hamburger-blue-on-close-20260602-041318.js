    ...
    if (target) {
      ...
      if (id === 'weekly-win-plan') {
        setTimeout(() => {
          ...
          // Wire both possible generate buttons as fallbacks (with correct output target)
          const mainGenBtn = document.getElementById('generate-plan-btn');
          if (mainGenBtn && typeof window.generatePlan === 'function') {
            mainGenBtn.onclick = () => {
              console.log('[main.js fallback] MAIN Generate Plan button clicked');
      ...
      // Book Vault — ensure books and category filters are rendered when navigating to the section
      if (id === 'books') {
        setTimeout(() => {
