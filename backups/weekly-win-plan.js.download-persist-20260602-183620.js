...
function downloadPlanWord() {
    // Use html2pdf for reliable direct-to-Downloads-folder PDF (no save dialog prompt in normal browser settings).
    // Falls back to the old .doc blob method only if html2pdf is not available.
    const preview = document.getElementById('plan-preview');
    if (!preview) return;

    if (typeof window.html2pdf === 'function') {
        const filename = '2026_Business_Plan.pdf';
        const opt = {
            margin: 0.6,
            filename: filename,
        ...
        // .save() triggers immediate download to the browser's default Downloads folder
        window.html2pdf().set(opt).from(preview).save();
        return;
    }

    // Fallback (old method) - still tries direct download via <a download>
    const header = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>2026 Business Plan</title><style>body{font-family:Calibri,Arial,sans-serif;margin:40px;line-height:1.6;color:#000;}' +
                   ...
                   'ul{padding-left:30px;}li{margin:12px 0;}</style></head><bo' + 'dy>';
    const content = preview.innerHTML;
    const footer = '</bo' + 'dy></ht' + 'ml>';
...
function initWeeklyWinPlan() {
    ...
    setTimeout(() => {
      ...
      if (out) {
        ...
        // Update legacy download button labels (content is the same, now PDF)
        out.querySelectorAll('button[onclick*="downloadPlanWord"]').forEach(btn => {
          const txt = (btn.textContent || '').toLowerCase();
          if (txt.includes('.doc') || txt.includes('download .doc')) {
