...
function wireGeneratePlanButton() {
  const handler = (ev) => {
    try {
      ...
      // Extra ultra force of the loading overlay right in the handler (before calling generate)
      // so user always sees *something* even if generatePlan has an early sync error.
      ...
      if (typeof generatePlan === 'function') {
        ...
        console.warn('[weekly-win-plan] generatePlan not found on window or in scope');
        if (le) {
  ...
  // 1. Direct on the button (if present now)
  ...
  // 2. Event delegation on the planning section (or body fallback) — this survives any future clone/replace of the button
