...
(function () {
  ...
  function socialQualityRulesBlock(kind, extra) {
    ...
    const lines = [
      ...
      '- Rotate: local pick (coffee / park / trail), seasonal how-to, mortgage question, one-word poll tied to THAT week.',
      ...
      'F. MIX: Stay in mortgage world on value days. Cap product posts — do not make 12 of 30 "cash-out idea" + "run numbers." Aim ~70% personal/local/relationship, 30% professional value. Only CHECKED themes. Do not write as a listing agent.',
  ...
  /** Parse three caption options — prefer Option N: markers; fall back to --- separators. */
...
function createOverviewSheet(year) {
    const data = [
        ...
        ["Built for Loan Officers in Fort Wayne & Beyond — Consistency Made Simple", "", "", "", "", "", "", ""],
        [],
        ["Core Strategy", "", "", "", "", "", "", ""],
...
function createMonthSheet(month, year, monthIndex, weekStart) {
    ...
    month.days.forEach(day => {
        ...
        let hashtags = '#LoanOfficerLife #IndianaRealEstate #Fort WayneIN';
...
HOBBY / CALENDAR RESTRAINT: Hobbies are seasoning. Follow the variety caps above. Most days are local, seasonal, people, or the job. Cap product posts (~30% professional value).
...
Generate the COMPLETE table now with all ${daysInMonth} days.`;
    ...
    try {
        ...
        const rawPlan = await window.callGrokAPI(null, {
            messages: [
                { role: 'system', content: 'You are an expert social media strategist for loan officers. Follow date/season sports rules, variety caps, and keep days 25–31 as substantial as days 1–7. Never invent the user\'s weekend. Cap product posts.' },
