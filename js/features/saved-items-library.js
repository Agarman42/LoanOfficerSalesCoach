...
(function () {
  ...
  function getUseNextLinks(item) {
    ...
    if (t === 'social' || t === 'blog') {
      links.push({ label: 'Social Post Creator', onclick: closeViewer + "if(typeof window.showSection==='function')window.showSection('social');", style: 'accent' });
    }
    ...
    if (t === 'underwriting') {
      links.push({ label: 'Underwriting Coach', onclick: closeViewer + "if(typeof window.showSection==='function')window.showSection('underwriting');", style: 'primary' });
    }
    if (t === 'mindset') {
      links.push({ label: 'Mindset Lab', onclick: closeViewer + "if(typeof window.showSection==='function')window.showSection('mindset-lab');", style: 'accent' });
    }
    if (t === 'plan') {
  ...
  window.viewSavedItem = function viewSavedItem(vaultIndex) {
    ...
    } else if (['equity-opportunity', 'equity-scan', 'underwriting', 'coach', 'social', 'script', 'plan', 'blog', 'postclosing', 'nurture', 'process'].includes(item.type)) {
