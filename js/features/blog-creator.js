...
(function () {
  ...
  function getBlogSecondaryActionsHtml() {
    // Quieter secondary row — no filled navy competing with the primary CTA
    return `
  ...
  function clarifyRuoffPublishLabels(html) {
    ...
    // Full rebuild of action strip when secondary hierarchy block is missing
  ...
  function patchRestoredBlogOutput(html) {
    ...
    patched = patched.replace(/Publish Kit/g, 'Next Steps');
    patched = patched.replace(/fa-rocket/g, 'fa-list-check');
    patched = patched.replace(/<div id="blog-seo-panel"[\s\S]*?<\/div>\s*(?=<!-- Social Caption Card)/, '');
...
function copyBlogAndJumpToPublisher() {
    // Open on the same click (before alert) so the popup is not blocked.
...
function wireBlogNextStepsButton() {
  ...
  nextStepsBtn.onclick = function () {
    ...
    // 1) Prefer feature-loader blog bundle (includes publish-kit)
    ...
    // 2) Fallback: inject publish-kit directly (blog already marked loaded without kit)
...
window.copyGooglePostWithFormatting = function copyGooglePostWithFormatting() {
  ...
  function wireBlogHowThisWorksPanel() {
    ...
    if (!el) return;
    const key = 'blog-how-this-works-seen-lo';
    let seen = false;
