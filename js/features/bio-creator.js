...
(function () {
  ...
  const STARTERS = {
    'loves-most': [
      { label: 'First-time buyer win', text: 'I love the moment a nervous first-time buyer realizes homeownership is actually possible — especially when we map out a clear path instead of overwhelming them with jargon.' },
    ...
    diff: [
      { label: 'Agent favorite', text: 'Realtors refer me because I communicate early and often, protect their reputation at the closing table, and never go silent when things get complicated.' },
      { label: 'Speed + clarity', text: 'Fast, accurate pre-approvals paired with plain-English explanations — clients know exactly where they stand and what happens next.' }
    ],
    family: [
    ...
    hobbies: [
      { label: 'Golf & outdoors', text: "When I'm not closing loans, I'm usually on the golf course or hiking trails around [your market]." },
  ...
  const DESTINATIONS = {
    ...
    'zillow': {
      ...
      pasteTip: 'Zillow Lender Hub → Profile → About Me'
    },
    'linkedin': {
...
- Before returning, mentally count ${unit}. If under ${targetMin}, add another concrete detail from the inputs.`;
  ...
  function buildPrompt(feedback, previousBio) {
    ...
    const companyNote = draft.mentionCompany
      ? 'You may mention the mortgage company by name if it fits naturally.'
      : 'Do NOT name a specific mortgage company unless the user explicitly asked in additional notes.';
    ...
    return `You are an elite mortgage-industry copywriter creating a platform-specific loan officer bio optimized for human trust AND search/AI discoverability.
