...
assert(s.mortgageInterest.keepMethod === 'actual_payment', 'keep interest uses actual P&I');
assert(s.mortgageInterest.keepMonthlyPiUsed === 1900, 'keep P&I is derived 1900');
...
console.log('\nCash-out LTV + escrow integrity + break-even gate');
const pureCashOut = C.computeScenario({
  homeValue: 450000,
  currentBalance: 320000,
  currentRate: 6.75,
  yearsRemaining: 27,
...
assert(pureCashOut.isCashOutScenario === true, 'pure cash-out flagged without consumer debts');
assert(pureCashOut.maxLtvPct === 80, 'pure cash-out uses 80% LTV');
assert(pureCashOut.overMaxLoan === true, '400k over 80% of 450k is over max');

const tinyCf = C.computeScenario(C.DEFAULTS);
assert(tinyCf.monthlyCashFlowChange > 0 && tinyCf.monthlyCashFlowChange < 50, 'default CF is tiny');
assert(tinyCf.breakEvenMonths == null, 'tiny CF → break-even not shown');
assert(tinyCf.breakEvenNotMeaningful === true, 'tiny CF marked not meaningful');

const weakEscrow = C.computeScenario(Object.assign({}, C.DEFAULTS, { taxes: 0, insurance: 0 }));
assert(weakEscrow.escrowDataWeak === true, 'zero T&I with escrow included is weak');
assert(weakEscrow.monthlyCashFlowChange > 100, 'weak escrow inflates CF (caller must warn)');

console.log('\nManual new PMI override');
const autoPmi = C.computeScenario(Object.assign({}, C.DEFAULTS, { newLoanAmount: 400000 }));
const manPmi = C.computeScenario(Object.assign({}, C.DEFAULTS, {
  newLoanAmount: 400000,
  newPmiManual: true,
  newPmi: 50
}));
assert(autoPmi.newPmi > 50, 'auto PMI estimate for high LTV');
assert(manPmi.newPmi === 50, 'manual PMI override sticks');
assert(manPmi.newHousing < autoPmi.newHousing, 'manual lower PMI reduces new housing');
assert(manPmi.newPmiIsManual === true, 'manual flag on scenario');
